...
fn emit_log(app: &AppHandle, level: &str, message: impl Into<String>) {
    ...
    diagnostics::record("xray", level, &message);
    let _ = app.emit(
        LOG_EVENT,
        LogEvent {
            line: format!("[xray] {message}"),
...
fn write_config(app: &AppHandle, port: u16, aether_binary: &Path) -> Result<PathBuf, AetherError> {
    let dir = runtime_dir(app);
    fs::create_dir_all(&dir).map_err(|e| AetherError::XrayConfigFailed(e.to_string()))?;
    let content = config::generate_config(port, aether_binary)
        .map_err(|e| AetherError::XrayConfigFailed(e.to_string()))?;
    let path = dir.join("xray-config.json");
    fs::write(&path, content).map_err(|e| AetherError::XrayConfigFailed(e.to_string()))?;
    Ok(path)
}
...
pub fn start_tunnel(
...
) -> Result<(), AetherError> {
    {
        ...
        if mgr.process.is_some() {
            return Err(AetherError::XrayAlreadyRunning);
        }
    }
    ...
    emit_log(
        ...
        "info",
        format!(
            "validated native TUN config with core={} and aether={}",
            binary.display(),
            aether_binary.display()
        ),
    );

    emit_log(&app, "info", "starting Xray native TUN process");
    ...
    emit_log(&app, "info", format!("native TUN process started (pid {pid})"));
    ...
    let deadline = Instant::now() + crate::singbox::status::TUN_STARTUP_TIMEOUT;
    loop {
        ...
        if let Some(exit) = process_exit_status(&manager)? {
            emit_log(
                &app,
                "error",
                format!("native TUN process exited during startup ({exit})"),
            );
            stop_tunnel(&app, &manager);
            return Err(AetherError::TunHealthFailed(format!(
                "Xray exited during startup ({exit})"
            )));
        }

        let health_error = match crate::singbox::status::verify_tunnel(aether_socks_port) {
            Ok(()) => {
                manager.lock().unwrap().active = true;
                emit_log(&app, "info", "system-wide Xray TUN data plane verified");
                return Ok(());
            }
        ...
        if Instant::now() >= deadline {
            emit_log(
                &app,
                "error",
                format!("Xray TUN data-plane verification timed out: {health_error}"),
            );
            stop_tunnel(&app, &manager);
...
pub fn process_exit_status(
    ...
    match mgr.process.as_mut() {
        Some(process) => process
            .try_wait()
            .map_err(|e| AetherError::Internal(format!("check Xray process: {e}"))),
        None => Ok(None),
    }
}

pub fn verify_active_tunnel(manager: &Arc<Mutex<XrayManager>>) -> Result<(), AetherError> {
    let port = {
        let mgr = manager.lock().unwrap();
        if !mgr.active {
    ...
    crate::singbox::status::verify_tunnel(port)
}

pub fn stop_tunnel(app: &AppHandle, manager: &Arc<Mutex<XrayManager>>) {
    let mut process = {
        let mut mgr = manager.lock().unwrap();
        mgr.active = false;
        mgr.socks_port = None;
    ...
    clear_pid(app);
    if had_process {
        emit_log(app, "info", "Xray TUN stopped");
    }
}

pub fn shutdown_blocking(manager: &Arc<Mutex<XrayManager>>, data_dir: &Path) {
    let mut process = {
        let mut mgr = manager.lock().unwrap();
        mgr.active = false;
        mgr.socks_port = None;
    ...
    let _ = fs::remove_file(data_dir.join("tun").join("xray.pid"));
}

fn expected_xray_name(name: &str) -> bool {
    let name = name.to_ascii_lowercase();
    if cfg!(windows) {
        name == "xray.exe" || (name.starts_with("xray-v") && name.ends_with(".exe"))
    } else {
        name == "xray" || name.starts_with("xray-v")
    }
}
        ...
        command
            ...
            .map(|output| {
                String::from_utf8_lossy(&output.stdout).lines().any(|line| {
                    line.split(',')
                        .next()
                        .map(|name| expected_xray_name(name.trim_matches('"')))
                        .unwrap_or(false)
                })
            })
            .unwrap_or(false)
        ...
        Command::new("ps")
            ...
            .map(|output| {
                String::from_utf8_lossy(&output.stdout).lines().any(|line| {
                    Path::new(line.trim())
                        .file_name()
                        .map(|name| expected_xray_name(&name.to_string_lossy()))
                        .unwrap_or(false)
                })
            })
            .unwrap_or(false)
...
pub fn reap_orphan(app: &AppHandle) {
    ...
    if !expected_process_is_alive(pid) {
        diagnostics::record(
            "xray",
            "info",
            format!("stale PID file ignored because PID {pid} is not an owned Xray core"),
        );
        clear_pid(app);
        return;
    }

    diagnostics::record("xray", "warn", format!("reaping owned orphan PID {pid}"));
    if kill_pid(pid) {
        clear_pid(app);
        diagnostics::record("xray", "info", format!("orphan PID {pid} terminated"));
    } else {
        diagnostics::record(
            "xray",
            "warn",
